# CuTeDSL FFI for JAX's CUDA PJRT plugin

JAX's CUDA PJRT plugin registers these FFI targets:

- `__xla_gpu_cutedsl_call_v3`
- `__xla_gpu_cutedsl_call_no_cuda_graph_v3`
- `__xla_gpu_cutedsl_collective_v3`

Version 3 is the only supported contract. Each target accepts `module` bytes
and their 32-byte SHA-256 `key` and passes XLA buffers to the compiled
`cutlass_call` entry point using CuTeDSL's `JaxArray` layout. The two
buffer-only targets create the module during prepare and use an explicit no-op
initialize stage.
...
address table. The first FFI result is an internal
...
the `//jaxlib/cuda/cutedsl:cutedsl_runtime` label flag; both FFIs
...
```text
--//jaxlib/cuda/cutedsl:cutedsl_runtime=@cutedsl_runtime//:runtime
```
...
```text
--//jaxlib/cuda/cutedsl:cutedsl_runtime=@cutedsl_runtime//:runtime
--//jaxlib/cuda/cutedsl:cutedsl_runtime_is_shared=true
--//jaxlib/cuda/cutedsl:cutedsl_runtime_dso=@cutedsl_runtime//:runtime_dso
```
