...
#include "jaxlib/cuda/cutedsl/collective_config.h"
...
#include "jaxlib/cuda/cutedsl/module.h"
#include "xla/ffi/ffi.h"

namespace jax::cuda::cutedsl {
...
constexpr std::array<absl::string_view, 10> kRequiredAttributeNames = {
    "schema_version",
    "abi_clique_size",
    "group_mode",
    "communication_id",
...
absl::Status ValidateRequiredAttributes(const ffi::Dictionary& attributes) {
  for (absl::string_view name : kRequiredAttributeNames) {
...
absl::StatusOr<std::vector<ReplicaGroup>> ParseReplicaGroups(
  ...
  if (offsets.back() < 0 ||
      static_cast<size_t>(offsets.back()) != members.size()) {
    return absl::InvalidArgumentError(
  ...
  for (size_t group_index = 0; group_index + 1 < offsets.size();
    ...
    if (begin < 0 || end <= begin ||
        static_cast<size_t>(end) > members.size()) {
      return absl::InvalidArgumentError(absl::StrFormat(
...
absl::StatusOr<CollectiveCallConfigV3> ParseCollectiveCallConfigV3(
  ...
  RETURN_IF_ERROR(ValidateRequiredAttributes(attributes));
  ...
  ASSIGN_OR_RETURN(ModuleImage module,
                   ModuleImage::Create(module_bytes, module_key));
